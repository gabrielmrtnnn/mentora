...
<section id="topBanner"
    ...
    <div class="max-w-7xl mx-auto flex items-center justify-between pl-16 pr-6 py-3">
        ...
        @guest
            <a href="{{ route('auth.redirect') }}" class="bg-yellow-400 text-black text-sm font-semibold px-4 py-2 rounded-lg">
                Mulai belajar
            </a>
        @endguest

        @auth
            <a href="{{ route('study-room') }}" class="bg-yellow-400 text-black text-sm font-semibold px-4 py-2 rounded-lg">
                Mulai belajar
            </a>
        @endauth
    </div>
